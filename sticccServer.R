...
observeEvent(input$computeTrajectories, {
  withBusyIndicatorServer("computeTrajectories",{
    ...
    withProgress(message = 'Computing trajectories', value = 0.25, {
...
observeEvent(input$plotTrajectories, {
  withBusyIndicatorServer("plotTrajectories",{
    ...
    output$sticPCA <- renderPlot({ 
      ...
      vic <- isolate(sticVals$vic)
      
      # Plot resulting vectors with a function that only takes vic as input
      .plotTrajectories(vic = isolate(vic()),
                        plotLoadings = isolate(input$plotLoadings),
                        scalingFactor = isolate(input$scalingFactor),
                        loadingFactor = 3.5,
                        gridSmoothing = isolate(input$gridSmoothing))
